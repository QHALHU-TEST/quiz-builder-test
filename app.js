...
❎ Beta-blocker`;
  ...
  function switchTab(name){
    ...
    // ✅ Extend to handle Mistakes page without breaking anything else:
    const m = document.getElementById('mistakes');
    if (m) m.classList.toggle('hidden', name!=='mistakes');
  }
  ...
  function startSessionFrom(quizObj, labelPrefix="Active"){
    ...
    $("#activeQuizLabel").textContent = label;
    if (activeQuizLabelMobile) activeQuizLabelMobile.textContent = label;
    // ✅ Reset attempts for the Mistakes feature at the start of any session
    resetAttempts();
...
explain: ARBs end with -sartan.`;
  ...
  /* ===================== Mistakes: record on Next; show on Finish ===================== */
  /* Current-quiz only; finalize list on Finish; toggle Incorrect-only / All answers   */
  ...
  /* Collect the current question’s chosen inputs from the DOM and upsert the entry */
  function recordAttemptFromDom(q){
    const chosen = Array.from(document.querySelectorAll('#questionView input'))
      .filter(i => i.checked)
      .map(i => i.parentElement.dataset.choiceId);
  ...
  /* Render the Mistakes page (after Finish). “Incorrect only” is default filter */
  ...
  /* Toggle buttons in Mistakes page */
  document.addEventListener('click', (e)=>{
    if (!e.target) return;
    if (e.target.id === 'filterIncorrect'){
      showIncorrectOnly = true;
      e.target.classList.add('active');
      const o = document.getElementById('filterAllAnswers'); if (o) o.classList.remove('active');
      renderMistakes();
    }
    if (e.target.id === 'filterAllAnswers'){
      showIncorrectOnly = false;
      e.target.classList.add('active');
      const o = document.getElementById('filterIncorrect'); if (o) o.classList.remove('active');
      renderMistakes();
    }
  });

  /* Record on Next (in addition to existing handler) */
  ...
  /* Finalize & render on Finish (in addition to existing handler) */
  ...
  if (finishBtn){
    finishBtn.addEventListener('click', ()=>{
      ...
      renderMistakes();
      // (Leave tab switching manual to avoid changing your flow.)
      // If you want auto-open: switchTab('mistakes');
    });
  }

  /* Ensure Mistakes tab renders its content when opened */
  $$(".tab").forEach(t=>{
    t.addEventListener('click', ()=>{
      if (t.dataset && t.dataset.tab === 'mistakes') renderMistakes();
    });
  });
});
