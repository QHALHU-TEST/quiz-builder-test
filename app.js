...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Boot message
  statusBanner.textContent = "✅ Ready — mobile UI fixed; PDF/DOCX import; AI + offline; review mistakes.";
  ...
  // Storage
  const STORAGE_KEY = "quizBuilder.v16.mobile.fix.quizzes";
  ...
  // ===== File drop & reading (TXT / PDF / DOCX + URL from GitHub Raw) =====
  ...
  dropWrap.addEventListener('drop', async (e)=>{
    ...
    // URL dropped from a tab
    const items = e.dataTransfer.items ? Array.from(e.dataTransfer.items) : [];
    const urlItem = items.find(it => it.kind === 'string' && (it.type === 'text/uri-list' || it.type === 'text/plain'));
    if (!urlItem) { alert("Drop a file or a direct link to a .pdf / .docx / .txt"); return; }
    urlItem.getAsString(async (url)=>{ await fetchByUrl(url).catch(err=>alert(err.message)); });
  });
  ...
  const pasteUrlInput = $("#pasteUrlInput");
  pasteUrlBtn.addEventListener('click', ()=>{ pasteUrlInput.classList.toggle('hidden'); pasteUrlInput.focus(); });
  pasteUrlInput.addEventListener('keydown', async (e)=>{
    if (e.key!=='Enter') return;
    const url = e.target.value.trim(); if (!url) return;
    try{ await fetchByUrl(url); }catch(err){ alert(err.message); }
  });
  ...
  function githubBlobToRaw(u){
    try{
      ...
      if (url.hostname === 'github.com' && url.pathname.includes('/blob/')) {
        ...
        const org = parts[0], repo = parts[1], branch = parts[3];
  ...
  async function fetchByUrl(raw){
    let url = githubBlobToRaw(raw);
    let resp;
    try {
      resp = await fetch(url, { mode:'cors' });
    } catch {
      throw new Error("Couldn’t fetch that link (CORS blocked). Download the file first, then Choose File.");
    }
    if (!resp.ok) throw new Error(`HTTP ${resp.status} — use Raw/Download link`);
    const ct = resp.headers.get('content-type') || '';
    const blob = await resp.blob();
    let ext = /\.pdf(\?|$)/i.test(url) || /pdf/i.test(ct) ? '.pdf'
            : /\.docx(\?|$)/i.test(url) || /officedocument\.wordprocessingml\.document/i.test(ct) ? '.docx'
            : /\.(txt|md)(\?|$)/i.test(url) || /text\/plain/i.test(ct) ? '.txt'
            : '.bin';
    const name = (url.split('/').pop() || 'download') + ext;
    const file = new File([blob], name, { type: ct || 'application/octet-stream' });
  ...
  async function readChapterFile(file){
    ...
    try{
      ...
      alert("Could not read the file. If this is a scanned PDF (images only), please use a text/DOCX version or run OCR.");
      return;
    }
  ...
  async function extractTextFromPDF(arrayBuffer){
    ...
    try{
      ...
      const maxPages = Math.min(pdf.numPages, 80); // generous but safe
      let full = "";
      ...
      if (!full) throw new Error("No selectable text found (likely a scanned PDF).");
      return full;
    }catch(e){
  ...
  // ===== Copy Prompt (offline manual AI with ✅/❎ rules) =====
...
❎ Beta-blocker`;
  ...
  async function aiGenerateQuiz(text){
    const system = sysPrompt("You are a precise quiz generator. Output ONLY in the app's strict format with ✅/❎ and a single '-' line between questions. No extra commentary.");
...
Chapter:
${text.slice(0, 120000)}`;
    return await aiChat([{role:"system", content:system},{role:"user", content:user}]);
  }
...
Chapter:
${text.slice(0, 120000)}`;
    const out = await aiChat([{role:"system", content:system},{role:"user", content:user}]);
...
Chapter:
${text.slice(0, 120000)}`;
    return (await aiChat([{role:"system", content:system},{role:"user", content:user}])).trim();
  }

  // ===== Feature chooser: offline or AI (auto based on toggle) =====
  ...
  function parseRaw(){
    const raw = $("#rawInput").value.trim(); if(!raw){ statusBanner.textContent="ℹ️ Paste questions first"; return null; }
...
explain: ARBs end with -sartan.`;
  ...
  function renderQuestion(){
    ...
    $("#finishBtn").onclick = ()=>showSummaryAndReview();
  ...
  function showSummaryAndReview(){
    // Show review panel with toggle
    buildReview(false);
    $("#reviewPanel").classList.remove('hidden');
    $("#mistakesOnlyChk").onchange = (e)=> buildReview(e.target.checked);
  }

  function buildReview(mistakesOnly){
    ...
    head.innerHTML = `<div class="pill">Final score: <b style="margin-left:6px">${correctCt} / ${total}</b></div>
    ...
    session.questions.forEach((q, idx)=>{
      ...
      if (mistakesOnly && isCorrect) return;

      const card = document.createElement('div'); card.className='card';
      const title = document.createElement('div'); title.className='question';
      title.textContent = (idx+1)+". "+q.text + (isCorrect ? " — ✅ Correct" : " — ❌ Incorrect");
      card.appendChild(title);

      // User choices
      const userLine = document.createElement('div'); userLine.className='small';
      const chosen = ans ? ans.selectedIds : [];
      userLine.textContent = "Your selection: " + (chosen.length ? chosen.map(id=>q.choices.find(c=>c.id===id)?.text).filter(Boolean).join("; ") : "—");
      card.appendChild(userLine);

      // Correct answers
      const correctLine = document.createElement('div'); correctLine.className='small';
      correctLine.textContent = "Correct: " + correctIds.map(id=>q.choices.find(c=>c.id===id)?.text).filter(Boolean).join("; ");
      card.appendChild(correctLine);

      if (q.explanation){
        const ex = document.createElement('div'); ex.className='small';
  ...
  function makeQuestionStem(sentence){ const t = sentence.replace(/\s+/g,' ').trim(); return /^(what|which|when|where|why|how)\b/i.test(t) ? (t.endsWith('?')?t:t+'?') : "Which statement matches the chapter content?"; }
