...
explain: ARBs end with -sartan.`;
  ...
  // Wrap Next to record before advancing (keep your original handler)
  (function(){
    const next = document.getElementById('nextBtn');
    if (!next) return;
    const orig = next.onclick;
    next.onclick = ()=>{
      try{
  ...
  // Wrap Finish to record + finalize (keep your original)
  (function(){
    const finish = document.getElementById('finishBtn');
    if (!finish) return;
    const orig = finish.onclick;
    finish.onclick = ()=>{
      try{
        if (session && session.questions && session.questions[session.idx]){
          recordAttemptFromDom(session.questions[session.idx]);
  ...
  // Wrap startSessionFrom to reset attempts on each new quiz (no layout change)
  (function(){
    const originalStart = startSessionFrom;
    if (typeof originalStart === 'function'){
      startSessionFrom = function(...args){
        resetAttempts();
  ...
  // Extend switchTab to show/hide the Mistakes section without changing other pages
  (function(){
  ...
  // Ensure clicking the Mistakes tab renders its content
  $$(".tab").forEach(t=>{
