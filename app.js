...
document.addEventListener('DOMContentLoaded', () => {
  ...
  statusBanner.textContent = "✅ Ready — PDF/DOCX/Text import; AI controls; Quiz/Flashcards/Summary.";
  ...
  // Storage (quizzes only)
  const STORAGE_KEY = "quizBuilder.v14.ai.controls.quizzes";
  function safeLoad(){ try{ const raw = localStorage.getItem(STORAGE_KEY); return raw ? JSON.parse(raw) : []; } catch(e){ showError("localStorage load failed: "+e); return []; } }
  function safeSave(arr){ try{ localStorage.setItem(STORAGE_KEY, JSON.stringify(arr)); } catch(e){ showError("localStorage save failed: "+e); } }
  ...
  document.body.addEventListener('click', (e)=>{
    if (e.target?.id==="menuBtn") document.body.classList.add('drawer-open');
    if (e.target?.id==="closeDrawerBtn") document.body.classList.remove('drawer-open');
    if (e.target.closest(".main") && window.innerWidth<=1000) document.body.classList.remove('drawer-open');
  });
  ...
  // ===== Home: Drop / Choose with PDF + DOCX support =====
  ...
  async function readChapterFile(file){
    ...
    try{
      if (/\.(txt|md)$/i.test(lower)){
        ...
        loadedText = await extractTextFromPDF(await file.arrayBuffer());
      } else if (/\.docx$/i.test(lower)){
        loadedText = await extractTextFromDOCX(await file.arrayBuffer());
      } else {
        alert("Please provide a .txt, .md, .pdf, or .docx file.");
        return;
      }
    }catch(err){
      showError("Read failed: "+err);
      alert("Could not read the file. See error box for details.");
      return;
    }
  ...
  async function extractTextFromPDF(arrayBuffer){
    ...
    for (let p=1; p<=maxPages; p++){
      ...
      const strings = content.items.map(it => ('string'===typeof it.str ? it.str : '')).filter(Boolean);
      full += strings.join(' ') + "\n";
    }
...
❎ Beta-blocker`;
  $("#copyPromptBtn").addEventListener('click', async ()=>{
    try{ await navigator.clipboard.writeText(GENERATION_PROMPT); statusBanner.textContent = "ℹ️ Prompt copied ✅"; }
    catch(e){ showError(e); alert("Copy failed"); }
  });

  // ===== AI Settings (key/model/temp/maxTokens/systemPrompt/useAiDefault) =====
  ...
  tempSlider.value = localStorage.getItem("openaiTemp") || "0.7";
  ...
  clearKeyBtn.onclick = ()=>{
    ...
    statusBanner.textContent = "🔐 API key cleared";
    apiKeyInput.value = "";
  };
  ...
  maxTokensInput.onchange = ()=>{
    ...
    maxTokensInput.value = v;
    localStorage.setItem("openaiMaxTokens", v.toString());
  };
  ...
  function aiDefaultEnabled(){ return hasApiKey() && localStorage.getItem("openaiUseDefault")==="1"; }

  // ===== AI core fetch =====
  async function aiChat(messages, opts={}){
    const apiKey = localStorage.getItem("openaiKey");
    if (!apiKey) { alert("Please enter your OpenAI API key in the sidebar first."); throw new Error("Missing API key"); }
  ...
  // ===== AI tools (use custom system prompt if provided) =====
  function sysPrompt(defaultText){
    const custom = localStorage.getItem("openaiSystem") || "";
    return (custom && custom.trim().length>0) ? custom : defaultText;
  }

  async function aiGenerateQuiz(text){
    const system = sysPrompt("You are a precise quiz generator. Output ONLY in the app's strict format with ✅/❎ and single '-' line between questions. No extra commentary.");
...
${text.slice(0, 120000)}`;
  ...
  async function aiGenerateFlashcards(text){
    const system = sysPrompt("You generate clean study flashcards.");
...
${text.slice(0, 120000)}`;
  ...
  async function aiSummarize(text){
    const system = sysPrompt("You are a study assistant that writes faithful, concise summaries.");
...
${text.slice(0, 120000)}`;
  ...
  // ===== Feature chooser (AI + offline wiring) =====
  $("#aiQuizBtn").addEventListener('click', async ()=>{
    try{
      statusBanner.textContent = "🤖 Generating quiz…";
      const aiText = await aiGenerateQuiz(loadedText);
      $("#rawInput").value = aiText;
  ...
  $("#aiFlashBtn").addEventListener('click', async ()=>{
    try{
      statusBanner.textContent = "🤖 Generating flashcards…";
      const cards = await aiGenerateFlashcards(loadedText);
      renderFlashcards(cards.map(c=>({ id: uid(), front: c.front, back: c.back })));
      switchTab('flashcards');
      statusBanner.textContent = "✅ AI flashcards ready";
    }catch(err){ showError(err); alert("AI Flashcards failed: "+err.message); statusBanner.textContent="⚠️ AI Flashcards failed"; }
  });

  $("#aiSummaryBtn").addEventListener('click', async ()=>{
    try{
      statusBanner.textContent = "🤖 Summarizing…";
      const sum = await aiSummarize(loadedText);
      $("#summaryText").textContent = sum;
      switchTab('summary');
      statusBanner.textContent = "✅ AI summary ready";
    }catch(err){ showError(err); alert("AI Summary failed: "+err.message); statusBanner.textContent="⚠️ AI Summary failed"; }
  });

  // Default buttons — respect "Use AI by default"
  $("#genQuizBtn").addEventListener('click', async ()=>{
    if (aiDefaultEnabled()){
      $("#aiQuizBtn").click();
    } else {
      const quiz = buildQuizFromText(loadedText);
      loadQuizIntoBuilder(quiz); switchTab('builder');
      $("#quizName").value = quiz.name || "Generated Quiz";
    }
  });
  $("#genFlashBtn").addEventListener('click', async ()=>{
    if (aiDefaultEnabled()){
      ...
      const cards = buildFlashcardsFromText(loadedText);
      renderFlashcards(cards); switchTab('flashcards');
    }
  });
  $("#genSummaryBtn").addEventListener('click', async ()=>{
    if (aiDefaultEnabled()){
      ...
      const sum = summarizeText(loadedText);
      $("#summaryText").textContent = sum; switchTab('summary');
    }
  });
  ...
  function parseRaw(){
    ...
    if(!raw){ statusBanner.textContent="ℹ️ Paste questions first"; return null; }
  ...
  $("#parseBtn").onclick = ()=>{
    const draft = parseRaw();
    if (draft){ parsedDraft = draft; renderPreview(draft); statusBanner.textContent="ℹ️ Parsed "+draft.questions.length+" question(s)"; }
  };
  $("#testRunBtn").onclick = ()=>{
    const draft = parseRaw();
    if (!draft){ statusBanner.textContent="ℹ️ Parse your text first"; return; }
    startSessionFrom(draft, "Testing (unsaved)"); switchTab('player');
  };
...
explain: ARBs end with -sartan.`;
  ...
  async function saveAsJSON(filename, dataObj){
    ...
    if (window.showSaveFilePicker) {
      try{
        ...
        await stream.close();
        statusBanner.textContent = "ℹ️ Saved to chosen location";
        return;
      }catch(err){ if (err?.name !== 'AbortError') showError("Save As failed: "+err); }
    }
    downloadBlob(filename, text);
    statusBanner.textContent = "ℹ️ Downloaded JSON";
  }
  ...
  function saveOrOverwrite(){
    ...
    if (!editingSame && existingByTitle){
      const ok = confirm(`A quiz named "${existingByTitle.name}" exists.\nOverwrite with current content?`);
      if (!ok){ statusBanner.textContent="ℹ️ Save canceled"; return; }
      ...
      safeSave(quizzes); renderQuizList(); statusBanner.textContent="ℹ️ Overwritten";
      return;
    }
    ...
    activeId = toSave.id;
    safeSave(quizzes); renderQuizList(); statusBanner.textContent= existsById ? "ℹ️ Saved (overwritten)" : "ℹ️ Saved";
  }
  function saveAsJSONAuto(){
    ...
    if (!data){
      ...
      if (!q){ statusBanner.textContent="ℹ️ Nothing to save"; return; }
  ...
  $("#importFile").addEventListener('change', (e)=>{
    ...
    reader.onload = ()=>{
      try{
        ...
        safeSave(quizzes); renderQuizList(); statusBanner.textContent = "ℹ️ Imported "+arr.length+" quiz(es)";
  ...
  $("#clearAllBtn").onclick = ()=>{
    if (confirm("Remove ALL saved quizzes on this device?")){ quizzes=[]; activeId=null; safeSave(quizzes); renderQuizList(); }
  };

  // ===== Offline Generators (heuristics) =====
  function buildQuizFromText(text){
    ...
    const questions = selected.map((s)=>{
      ...
      const choices = shuffle([{text:correct, correct:true}]
        .concat(distractors.map(t=>({text:t, correct:false}))))
        .slice(0, Math.max(2, Math.min(4, 1 + distractors.length)));
      const qText = makeQuestionStem(s);
      return { id: uid(), text: qText, explanation:"", choices };
    });
  ...
  function makeQuestionStem(sentence){
    const trimmed = sentence.replace(/\s+/g,' ').trim();
    if (/^(what|which|when|where|why|how)\b/i.test(trimmed)) return trimmed.endsWith('?') ? trimmed : trimmed + '?';
    return `Which statement matches the chapter content?`;
  }
  function rankSentences(sentences){
    ...
    const score = s=>{
      ...
      return sc / Math.sqrt(s.length+1);
    };
  ...
  function buildFlashcardsFromText(text){
    ...
    const terms = topTerms(text, 20);
    return terms.map(term=>{
      const def = sentences.find(s=> new RegExp(`\\b${escapeReg(term)}\\b`,'i').test(s)) || "";
      return { id: uid(), front: term, back: def || "Definition/example from chapter." };
    });
  }
  function topTerms(text, k){
    ...
    text.toLowerCase().replace(/[^a-z0-9\s]/g,' ').split(/\s+/).forEach(w=>{
      if (!w || stop.has(w) || w.length<4) return;
      freq[w] = (freq[w]||0)+1;
    });
  ...
  function summarizeText(text){
    ...
    const take = Math.min(12, Math.max(4, Math.floor(sentences.length/8)));
  ...
  function renderFlashcards(cards){
    ...
    cards.forEach(c=>{
      const card = document.createElement('div'); card.className='card';
      const front = document.createElement('div'); front.innerHTML = "<b>"+esc(c.front)+"</b>";
      const back  = document.createElement('div'); back.className='small'; back.textContent = c.back;
