...
explain: ARBs end with -sartan.`;
  ...
  // ===== Attempts (current quiz only) =====
  let attemptsCurrent = [];
  let filterTypeMistakesOnly = true; // default: Incorrect only
  let attemptsFinalized = false;

  function recordAttemptFromDom(q){
    // Collect chosen ids from current question DOM
    ...
    // Upsert by question id (only one attempt per question kept)
    const idx = attemptsCurrent.findIndex(a=>a.question===q.text);
    if (idx>=0) attemptsCurrent[idx] = entry; else attemptsCurrent.push(entry);
  }
  ...
  function renderMistakes(){
    ...
    const source = filterTypeMistakesOnly ? attemptsCurrent.filter(a=>!a.correct) : attemptsCurrent;
  ...
  document.addEventListener('click', (e)=>{
    if (!e.target) return;
    if (e.target.id==='typeMistakes'){ filterTypeMistakesOnly=true; e.target.classList.add('active'); const t2=document.getElementById('typeAll'); if(t2) t2.classList.remove('active'); renderMistakes(); }
    if (e.target.id==='typeAll'){ filterTypeMistakesOnly=false; e.target.classList.add('active'); const t1=document.getElementById('typeMistakes'); if(t1) t1.classList.remove('active'); renderMistakes(); }
  });

  // Ensure larger paste box height if CSS not applied
  const ri = document.getElementById('rawInput'); if (ri && (!ri.style.minHeight||ri.style.minHeight==='')){ ri.style.minHeight='420px'; ri.style.fontSize='16px'; }

  // Sidebar safety: ESC to close
