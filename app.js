...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function switchTab(name){
    const ids = ["home","builder","player","flashcards","summary","mistakes"];
    let shown = false;
    ids.forEach(id=>{
    ...
    // Fallback: if target missing, show first available section
    if (!shown){
      for (const id of ids){
        const sec = document.getElementById(id);
        const btn = document.querySelector(`.tab[data-tab="${id}"]`);
        if (sec){ sec.classList.remove('hidden'); if (btn) btn.classList.add('active'); break; }
      }
    }
  });
  }
...
explain: ARBs end with -sartan.`;
  ...
  function renderQuestion(){
    ...
    q.choices.forEach(opt=>{
      ...
      input.onchange = ()=>{ const parent = input.closest('.opt'); if(parent){ parent.classList.toggle('selected', input.checked); } const parent = input.closest('.opt'); if(parent){ parent.classList.toggle('selected', input.checked); }
      ...
      try{ recordAttempt(q, chosen, isCorrect); }catch(e){}
        }
      };
...
// Save / Export
  ...
  // ===== Attempts / Mistakes tracking =====
  const STORAGE_ATTEMPTS = 'quizBuilder.v15.attempts';
  function loadAttempts(){ try{ const raw=localStorage.getItem(STORAGE_ATTEMPTS); return raw? JSON.parse(raw): []; }catch(e){ showError('Load attempts failed: '+e); return []; } }
  function saveAttempts(arr){ try{ localStorage.setItem(STORAGE_ATTEMPTS, JSON.stringify(arr)); }catch(e){ showError('Save attempts failed: '+e); } }
  let attempts = loadAttempts();
  let attemptsCurrent = [];
  let filterScopeAll = false;       // false = this quiz, true = all
  let filterTypeMistakesOnly = true;// true = mistakes only, false = all answers

  function recordAttempt(q, chosenIds, isCorrect){
    const entry = {
      id: uid(), at: new Date().toISOString(),
      quizId: session.quizId, quizName: session.name,
      question: q.text,
      choices: q.choices.map(c=>({ id:c.id, text:c.text, correct:c.correct })),
      chosen: chosenIds, correct: isCorrect
    };
    attempts.unshift(entry);
    attemptsCurrent.unshift(entry);
    if (attempts.length>1000) attempts = attempts.slice(0,1000);
    saveAttempts(attempts);
  }

  function getAttemptsSource(){
    const pool = filterScopeAll ? attempts : attemptsCurrent;
    if (!filterTypeMistakesOnly) return pool;
    return pool.filter(a=>!a.correct);
  }

  function renderMistakes(){
    const list = $("#mistakeList"); if (!list) return;
    list.innerHTML = "";
    const source = getAttemptsSource();
    if (!source.length){ list.innerHTML = "<div class='small'>No entries yet.</div>"; return; }
    source.forEach(m => {
      const card = document.createElement('div'); card.className = 'card';
      const header = document.createElement('div');
      header.innerHTML = `<div class="row wrap" style="justify-content:space-between;align-items:center">
      ...
      m.choices.forEach(c => {
        ...
        const isChosen = m.chosen.includes(c.id);
        row.classList.toggle('correct', c.correct);
        row.classList.toggle('wrong', isChosen && !c.correct);
        row.classList.toggle('selected', isChosen);
        row.innerHTML = `<span class="box"></span><span>${c.correct ? '✅ ' : (isChosen ? '❌ ' : '')}${esc(c.text)}</span>`;
  ...
  // Toolbar wiring
  $("#scopeCurrent") && ($("#scopeCurrent").onclick = ()=>{ filterScopeAll=false; document.getElementById('scopeCurrent').classList.add('active'); document.getElementById('scopeAll').classList.remove('active'); renderMistakes(); });
  $("#scopeAll") && ($("#scopeAll").onclick = ()=>{ filterScopeAll=true; document.getElementById('scopeAll').classList.add('active'); document.getElementById('scopeCurrent').classList.remove('active'); renderMistakes(); });
  $("#typeMistakes") && ($("#typeMistakes").onclick = ()=>{ filterTypeMistakesOnly=true; document.getElementById('typeMistakes').classList.add('active'); document.getElementById('typeAll').classList.remove('active'); renderMistakes(); });
  $("#typeAll") && ($("#typeAll").onclick = ()=>{ filterTypeMistakesOnly=false; document.getElementById('typeAll').classList.add('active'); document.getElementById('typeMistakes').classList.remove('active'); renderMistakes(); });

  // Export/Clear
  $("#exportMistakesBtn") && ($("#exportMistakesBtn").onclick = ()=>{
    const blob = new Blob([JSON.stringify({ exportedAt:new Date().toISOString(), attempts }, null, 2)], {type:'application/json'});
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'attempts_export.json'; document.body.appendChild(a); a.click(); a.remove();
    setTimeout(()=>URL.revokeObjectURL(url), 1200);
  });
  $("#clearMistakesBtn") && ($("#clearMistakesBtn").onclick = ()=>{
    if (confirm('Clear all saved attempts?')){ attempts=[]; attemptsCurrent=[]; saveAttempts(attempts); renderMistakes(); }
  });
