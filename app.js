...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $$(".tab").forEach(t=>t.addEventListener('click', ()=>{  const tab = t.dataset.tab; switchTab(tab); if (tab==='mistakes') renderMistakes(); }));
  ...
  async function readChapterFile(file){
    const isImage = /\.(png|jpe?g|webp|bmp|gif)$/i.test((file.name||'').toLowerCase());
    if (isImage){
      try{
      ...
      }catch(err){ showError('OCR failed: '+err); alert('OCR failed — see error box'); return; }
...
${text.slice(0, 120000)}`;
  ...
  function normalizeEmojiFormat(raw){
    // Turn leading '+'/'-' into ✅/❎ and keep existing emoji; do not touch other lines
    return raw.split(/\n/).map(line=>{
  ...
  function parseRaw(){
    let raw = $("#rawInput").value.trim(); if(!raw){ statusBanner.textContent="ℹ️ Paste questions first"; return null; }
    raw = normalizeEmojiFormat(raw);
...
explain: ARBs end with -sartan.`;
  ...
  function renderQuestion(){
    ...
    checkBtn.onclick = ()=>{
      ...
      if (!isCorrect) { try{ recordMistake(q, chosen); }catch(e){ /* silent */ } }
  ...
  function renderMistakes(){
    ...
    list.innerHTML = "";
    if (!mistakes.length){
      list.innerHTML = "<div class='small'>No mistakes recorded yet. Play a quiz and any wrong answers will show here.</div>";
      return;
    }
    mistakes.forEach(m => {
      const card = document.createElement('div'); card.className = 'card';
      ...
      m.choices.forEach(c => {
        ...
        row.innerHTML = `<span>${isCorrect ? '✅' : (isChosen ? '❌' : '•')} ${esc(c.text)}</span>`;
  ...
  $("#clearMistakesBtn") && ($("#clearMistakesBtn").onclick = ()=>{
    if (confirm('Clear all recorded mistakes?')){
      mistakes = []; saveMistakes(mistakes); renderMistakes();
    }
  });
// Save / Export
